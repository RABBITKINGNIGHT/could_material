...
class LDConv(nn.Module):
    def __init__(self, inc, outc, num_param, stride=1, bias=None):
        ...
        self.outc= outc
        self.inc = inc
        self.conv = nn.Sequential(nn.Conv3d(inc, outc, kernel_size=(num_param,1,1), stride=(num_param,1,1), bias=bias))
        self.conv2 = nn.Sequential(nn.BatchNorm2d(outc),
                                   nn.SiLU())
    ...
    def _get_x_q(self, x, q, N):
        ...
        # (b, c, h*w*N)
        index = index.contiguous().unsqueeze(dim=1).expand(-1, c, -1, -1, -1).view(b, c, -1)
